  ...
  return (
    ...
    >
        <div className="flex items-start justify-between p-6 border-b">
          <div>
            <h2 className="text-xl font-bold text-gray-900 dark:text-white">
              {t('eingabe.form.title', 'Neue Transaktion')}
            </h2>
            {!isModal && (
              <p className="text-sm text-gray-500 mt-1">
                {t('eingabe.form.subtitle', 'Erfasse manuell oder nutze den CSV-Upload auf dieser Seite.')}
              </p>
            )}
          </div>
          {isModal && (
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div>
            <Label htmlFor="date">{t('eingabe.form.date', 'Datum')}</Label>
            <Input
              id="date"
              type="date"
              value={formData.date}
          ...
          <div>
            <Label htmlFor="merchant">{t('eingabe.form.merchant', 'Händler / Geschäft *')}</Label>
            <Input
              id="merchant"
              type="text"
              placeholder={t('eingabe.form.merchant.placeholder', 'z.B. Coop, Zalando, SBB')}
          ...
          <div>
            <Label htmlFor="amount">{t('eingabe.form.amount', 'Betrag (CHF) *')}</Label>
            <Input
              id="amount"
              type="number"
              step="0.01"
          ...
          <div>
            <Label htmlFor="rawCategory">{t('eingabe.form.rawCategory', 'Kategorie (optional)')}</Label>
            <Input
              id="rawCategory"
              type="text"
              placeholder={t('eingabe.form.rawCategory.placeholder', 'z.B. Shopping, Lebensmittel')}
              value={formData.rawCategory}
              onChange={(e) => setFormData({ ...formData, rawCategory: e.target.value })}
            />
            <p className="text-sm text-gray-500 mt-1">
              {t(
                'eingabe.form.rawCategory.hint',
          ...
          <div>
            <Label htmlFor="justification">{t('eingabe.form.justification', 'Begründung (optional)')}</Label>
            <Textarea
              id="justification"
              placeholder={t('eingabe.form.justification.placeholder', 'Warum hast du diesen Kauf getätigt?')}
          ...
          {goals.length > 0 && (
            <div className="space-y-3">
              <Label>{t('eingabe.form.goal', 'Zuweisung zum Sparziel (optional)')}</Label>
              <Select
              ...
              >
                <SelectTrigger className="bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 border-gray-300 dark:border-gray-700">
                  <SelectValue placeholder={t('eingabe.form.goal.placeholder', 'Sparziel auswählen (optional)')} />
                </SelectTrigger>
              ...
              <Input
                type="number"
                step="0.01"
                placeholder={t('eingabe.form.allocate.placeholder', 'Betrag als Einzahlung (CHF)')}
          ...
          <div className={`flex ${isModal ? 'space-x-3' : 'justify-end'} pt-4`}>
            {isModal && (
              <Button
                ...
                disabled={loading}
              >
                {t('eingabe.form.cancel', 'Abbrechen')}
              </Button>
            )}
            <Button type="submit" className={isModal ? 'flex-1' : ''} disabled={loading}>
              {loading ? (
                <>
                  ...
                  {t('eingabe.form.loading', 'Analysiere...')}
                </>
              ) : (
                t('eingabe.form.submit', 'Transaktion erfassen')
              )}
            </Button>
          </div>
        </form>
